...
export default {
  ...
  methods: {
    ...
    async setCompleted(i, task){
      if(!task.completed){
        await window.db.update('tasks', { completed: this.date }, { id: task.id })
        ...
        await window.db.update('tasks', { completed: null }, { id: task.id })
    ...
    async remove(i, id, key){
      await window.db.delete('tasks', { id: id })
      this[key].splice(i, 1)
      this.sortTasks({
        ongoing: {
          key: 'deadline'
